...
**submodule package**
A stow package whose payload is or contains a git submodule pointing to an external repo. Examples: `neovim` (kickstart.nvim fork), `tmux/plugins/*`, `zsh-plugins/*`.
...
`~/.agents/skills/` — local (non-plugin) agent skill files. OpenCode (`~/.config/opencode/skills`) gets a symlink pointing here.
...
**run_once_ script**
A chezmoi script that executes exactly once per machine (tracked by a hash of the script's contents). Used for cloning external repos (neovim, TPM).
